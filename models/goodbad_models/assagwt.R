model {
    ...
    for(b in 1:2)
    {
      for(s in 1:N_sample_season)
      {
        for (g in 1:N_age_group)
        {
          ...
          for (wt in hosp_wardtypes)
          {
            logit(assagwt.prob[a,b,s,g,wt]) <- assag.effect[a,b,s,g] +
              wt.effect[wt]
          }
          # equivalent to assagwt.prob
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.sampleseason, sd.agegroup, sd.wt
}
