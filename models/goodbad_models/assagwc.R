      ...
      for(s in 1:N_sample_season)
      {
        for (g in 1:N_age_group)
        {
          ...
          for (w in hosp_wards)
          {
            ...
            {
              logit(assagwc.prob[a,b,s,g,w,c]) <- assag.effect[a,b,s,g] +
                w.effect[w] + clin.effect[c]
            }
          }
          # equivalent to assagw.prob
          logit(assag.gp.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + gpcw.effect
          logit(assag.v.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + vcw.effect
          logit(assag.o.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + ocw.effect
        }
      }
    }
  }
