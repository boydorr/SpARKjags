model {
  ...
  for (x in 1:N_chicken)
  {
    # probability of belonging to the bad group
    bad.chicken[x] ~ dbern(prob.of.bad.chicken)
    # index the bad group in a.prob
    index.bad.chicken[x] <- bad.chicken[x] + 1

    for(a in 1:antibiotic_classes)
    {
      # Response is different for each antibiotic and depending on
      # which pop it's from
      response_animal[chicken_GUID[x],a] ~ dbern(a.prob[a, index.bad.chicken[x]])
    }
  }
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, prob.of.bad.chicken, bad.p, bad.gp, bad.v, bad.o, bad.chicken, intercept, sd.class
}
