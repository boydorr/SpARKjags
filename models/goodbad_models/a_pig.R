model {
  ...
  {
    # probability of belonging to the bad group
    bad.pig[x] ~ dbern(prob.of.bad.pig)
    # index the bad group in a.prob
    index.bad.pig[x] <- bad.pig[x] + 1

    for(a in 1:antibiotic_classes)
    {
      # Response is different for each antibiotic and depending on
      # which pop it's from
      response_animal[pig_GUID[x],a] ~ dbern(a.prob[a, index.bad.pig[x]])
    }
  }
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, prob.of.bad.pig, bad.p, bad.gp, bad.v, bad.o, bad.pig, intercept, sd.class
}
