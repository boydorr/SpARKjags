model {
      ...
      for(a in 1:antibiotic_classes)
      {
        ...
        { # good bad
          ...
          for (c in c(ncarr, nclin)) # 1, 2!
          {
            for (w in hosp_wards)
            {
              logit(acsm.prob[a,b,c,s,g,w]) <-
                ac.effect[a,b,c] + ss.age.effect[s,g] +  w.effect[w]
            }
          }
  ...
  #monitor# full.pd, dic, deviance, a.prob,ac.prob, a.gp.prob, a.v.prob, a.o.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.clin, sd.sampleseason, sd.agegroup, sd.ward
}
