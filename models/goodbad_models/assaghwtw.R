model {
  ...
  # equivalent to nh.effect + wt.effect
  ...
  # equivalent to nh.effect + wt.effect + w.effect
        ...
        for (g in 1:N_age_group)
        {
          ...
          for (h in 1:N_hosp)
          {
            for (wt in hosp_wardtypes)
            {
              for (w in hosp_wards)
              {
                logit(assaghwtw.prob[a,b,s,g,h,wt,w]) <- assag.effect[a,b,s,g] +
                  h.effect[h] + wt.effect[wt] + w.effect[w]
              }
            }
          }

          # equivalent to assagwtw.prob
          logit(assag.gp.prob[a,b,s,g]) <- assag.effect[a,b,s,g] +
            nhwtw.gp.effect
          logit(assag.v.prob[a,b,s,g]) <- assag.effect[a,b,s,g] +
            nhwtw.v.effect
          logit(assag.o.prob[a,b,s,g]) <- assag.effect[a,b,s,g] +
            nhwtw.o.effect
        }
      }
    }
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.sampleseason, sd.agegroup, sd.hosp, sd.wt, sd.ward
}
