model {
  ...
  # Prior distribution for age.effect (since there is only one explanatory
  # variable, put intercept here; putting the intercept here rather than
  # above stops variables from covarying negatively)
  for (a in 1:N_sample)
  {
    logit(age.prob[a]) <- intercept + age.effect * age[a] +
      age2.effect * pow(age[a], 2)
  }
  ...
  #monitor# full.pd, dic, deviance, age.effect, age2.effect, intercept
}
