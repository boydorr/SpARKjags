model {
  ...
  #
  # Prior distribution for cw.effect (log-odds for each ward type in each
  ...
  for (c in c(ncarr, nclin)) # 1, 2!
  {
    for (w in hosp_wards)
    {
      # clinical effect is different for each ward
      cw.effect[w,c] ~ dnorm(clin.effect[c], tau)  # params = 119*2
      logit(wc.prob[w,c]) <- cw.effect[w,c]
    }
  }

  # equivalent to cw.effect
  gp.effect ~ dnorm(clin.effect[gp_clinical], tau) # params = 1
  logit(gp.prob) <- gp.effect

  v.effect ~ dnorm(clin.effect[v_clinical], tau)   # params = 1
  logit(v.prob) <- v.effect

  o.effect ~ dnorm(clin.effect[o_clinical], tau)   # params = 1
  logit(o.prob) <- o.effect

  # ------------------------

  # Prior values for precision
  tau ~ dgamma(0.001, 0.001)                       # params = 1
  ...
  #monitor# full.pd, dic, deviance, clin.effect, c.prob, gp.prob, v.prob, o.prob, odds.c, sd,
}
