model {
  ...
  {
    h_resist[p] ~ dbern(w.prob[ward[h_sample_GUID[p]]])
  }
  ...
  for (w in hosp_wards)
  {
    ward.effect[w] ~ dnorm(h.effect[hospital[w]], tau.ward)
    logit(w.prob[w]) <- ward.effect[w]
  }
  ...
  #monitor# full.pd, dic, deviance, intercept, h.prob, nh.prob, sd.hosp, sd.ward
}
