model {
  ...
  # Add another variable with levels hospital, non-hospital
  h.effect ~ dnorm(0, 0.001)
  nh.effect ~ dnorm(0, 0.001)
          ...
          for (st in sampletypes)
          {
            for(w in hosp_wards)
            {
              for (c in c(ncarr, nclin))
              {
                ...
                logit(aaggwst.prob[a,m,g,r,w,c,st]) <- asmagg.effect[a,m,g,r] +
                  ward.effect[w] + cst.effect[c,st] + h.effect
              }
            }

            logit(gp.prob[a,m,g,r,st]) <- asmagg.effect[a,m,g,r] +
              gpw.effect + gpcst.effect[st] + nh.effect
            logit(v.prob[a,m,g,r,st]) <- asmagg.effect[a,m,g,r] +
              vw.effect + vcst.effect[st] + nh.effect
            logit(o.prob[a,m,g,r,st]) <- asmagg.effect[a,m,g,r] +
              ow.effect + ocst.effect[st] + nh.effect
          }
        }
      }
    }
