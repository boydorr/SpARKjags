model {
  ...
  for (l in 1:N_livestock)
  {
    # probability of belonging to the bad group
    bad.livestock[l] ~ dbern(prob.of.bad.livestock)
    # index the bad group in a.prob
    index.bad.livestock[l] <- bad.livestock[l] + 1

    for(a in 1:antibiotic_classes)
    {
      # Response is different for each antibiotic and depending on
      # which pop it's from
      response_animal[livestock_GUID[l],a] ~ dbern(a.prob[a, index.bad.livestock[l]])
    }
  }

  for (x in 1:N_companion)
  {
    # probability of belonging to the bad group
    bad.companion[x] ~ dbern(prob.of.bad.companion)
    # index the bad group in a.prob
    index.bad.companion[x] <- bad.companion[x] + 1

    for(a in 1:antibiotic_classes)
    {
      # Response is different for each antibiotic and depending on
      # which pop it's from
      response_animal[companion_GUID[x],a] ~ dbern(a.prob[a, index.bad.companion[x]])
    }
  }
  ...
  {
    # probability of belonging to the bad group
    bad.wild[x] ~ dbern(prob.of.bad.wild)
    # index the bad group in a.prob
    index.bad.wild[x] <- bad.wild[x] + 1

    for(a in 1:antibiotic_classes)
    {
      # Response is different for each antibiotic and depending on
      # which pop it's from
      response_animal[wild_GUID[x],a] ~ dbern(a.prob[a, index.bad.wild[x]])
    }
  }
  ...
  prob.of.bad.livestock ~ dbeta(1, 1)
  prob.of.bad.companion ~ dbeta(1, 1)
  prob.of.bad.wild ~ dbeta(1, 1)
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, prob.of.bad.livestock, prob.of.bad.companion, prob.of.bad.wild, bad.p, bad.gp, bad.v, bad.o, bad.livestock, bad.companion, bad.wild, intercept, sd.class
}
