model {
      ...
      for(a in 1:antibiotic_classes)
      {
        ...
        { # good bad
          ...
          for (c in c(ncarr, nclin)) # 1, 2!
          {
            for (wt in hosp_wardtypes)
            {
              logit(acsm.prob[a,b,c,s,g,wt]) <-
                ac.effect[a,b,c] + ss.age.effect[s,g] + wt.effect[wt]
            }
          }

          logit(asm.gp.prob[a,b,s,g]) <-
            a.gp.effect[a,b] + ss.age.effect[s,g] + gp.effect
          logit(asm.v.prob[a,b,s,g]) <-
            a.v.effect[a,b] + ss.age.effect[s,g] + v.effect
          logit(asm.o.prob[a,b,s,g]) <-
            a.o.effect[a,b] + ss.age.effect[s,g] + o.effect
        }
      }
    }
  }
  ...
  #monitor# full.pd, dic, deviance, a.prob, ac.prob, a.gp.prob, a.v.prob, a.o.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.clin, sd.sampleseason, sd.agegroup, sd.wt
}
