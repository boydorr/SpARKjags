        ...
        for (g in 1:N_age_group)
        {
          ...
          for (st in sampletypes)
          {
            for (w in hosp_wards)
            {
              ...
              {
                logit(assagwst.prob[a,b,s,g,w,c,st]) <- assag.effect[a,b,s,g] +
                  ward.effect[w] + cst.effect[c,st]
              }
            }
            # equivalent to assagw.prob
            logit(assag.gp.prob[a,b,s,g,st]) <- assag.effect[a,b,s,g] +
              gpw.effect + gpcst.effect[st]
            logit(assag.v.prob[a,b,s,g,st]) <- assag.effect[a,b,s,g] +
              vw.effect + vcst.effect[st]
            logit(assag.o.prob[a,b,s,g,st]) <- assag.effect[a,b,s,g] +
