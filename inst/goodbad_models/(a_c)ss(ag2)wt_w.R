model {
  ...
  for (wt in hosp_wardtypes)
  {
    ...
    for (w in hosp_wards)
    {
      wtw.effect[wt,w] ~ dnorm(wt.effect[wt], tau.ward)
  ...
  wtw.gp.effect ~ dnorm(gp.effect, tau.ward)
  wtw.v.effect ~ dnorm(v.effect, tau.ward)
  wtw.o.effect ~ dnorm(o.effect, tau.ward)
        ...
        { # good bad
          ...
          for (c in c(ncarr, nclin)) # 1, 2!
          {
            for (wt in hosp_wardtypes)
            {
              for (w in hosp_wards)
              {
                logit(acsm.prob[a,b,c,s,g,wt,w]) <-
                  ac.effect[a,b,c] + ss.age.effect[s,g] + wtw.effect[wt,w]
              }
            }
          }

          logit(asm.gp.prob[a,b,s,g]) <-
            a.gp.effect[a,b] + ss.age.effect[s,g] + wtw.gp.effect
          logit(asm.v.prob[a,b,s,g]) <-
            a.v.effect[a,b] + ss.age.effect[s,g] + wtw.v.effect
          logit(asm.o.prob[a,b,s,g]) <-
            a.o.effect[a,b] + ss.age.effect[s,g] + wtw.o.effect
        }
      }
    }
  }
  ...
  #monitor# full.pd, dic, deviance, a.prob, ac.prob, a.gp.prob, a.v.prob, a.o.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.clin, sd.sampleseason, sd.agegroup, sd.wt, sd.ward
}
