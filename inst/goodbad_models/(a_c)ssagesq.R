model {
  ...
  for (n in 1:N_sample)
  {
    mu.age[n] <- age.effect * age[n] + age2.effect * pow(age[n], 2)
    ...
    { # good bad
      for (c in c(ncarr, nclin)) # 1, 2!
      {
        ...
        for(s in 1:N_sample_season)
        {
          for (n in 1:N_sample)
          {
            logit(acsm.prob[a,b,c,s,n]) <-
              ac.effect[a,b,c] + sampleseason.effect[s] + mu.age[n]
          }
        }
      }
      ...
      for(s in 1:N_sample_season)
      {
        for (n in 1:N_sample)
        {
          logit(asm.gp.prob[a,b,s,n]) <-
            a.gp.effect[a,b] + sampleseason.effect[s] + mu.age[n]
          logit(asm.v.prob[a,b,s,n]) <-
            a.v.effect[a,b] + sampleseason.effect[s] + mu.age[n]
          logit(asm.o.prob[a,b,s,n]) <-
            a.o.effect[a,b] + sampleseason.effect[s] + mu.age[n]
        }
      }
    }
  }
