model {
    ...
    for(b in 1:2)
    {
      for(s in 1:N_sample_season)
      {
        for (g in 1:N_age_group)
        {
          ...
          for (w in hosp_wards)
          {
            logit(assagwtw.prob[a,b,s,g,w]) <- assag.effect[a,b,s,g] +
              wtw.effect[w]
          }

          # equivalent to assagwtw.prob
          logit(assag.gp.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + gpw.effect
          logit(assag.v.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + vw.effect
          logit(assag.o.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + ow.effect
        }
      }
    }
  }
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.sampleseason, sd.agegroup, sd.wt, sd.ward
}
