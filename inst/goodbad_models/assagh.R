model {
    ...
    for(b in 1:2)
    {
      for(s in 1:N_sample_season)
      {
        for (g in 1:N_age_group)
        {
          ...
          for (h in 1:N_hosp)
          {
            logit(assagh.prob[a,b,s,g,h]) <- assag.effect[a,b,s,g] + h.effect[h]
          }
          # equivalent to assagh.prob
          logit(assagnh.prob[a,b,s,g]) <- assag.effect[a,b,s,g] + nh.effect
        }
      }
    }
  }
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.sampleseason, sd.agegroup, sd.hosp
}
