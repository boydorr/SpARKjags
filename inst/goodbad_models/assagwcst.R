model {
    ...
    for(b in 1:2)
    {
      for(s in 1:N_sample_season)
      {
        for (g in 1:N_age_group)
        {
          for (st in sampletypes)
          {
            assagst.effect[a,b,s,g,st] <- antibiotic.class.effect[a,b] +
              sampleseason.effect[s] + agegroup.effect[g] + st.effect[st]

            for (w in hosp_wards)
            {
              for (c in c(ncarr, nclin))
              {
                logit(assagw.prob[a,b,s,g,st,w,c]) <- assagst.effect[a,b,s,g,st] +
                  w.effect[w] + clin.effect[c]
              }
            }
            # equivalent to assagw.prob
            logit(assag.gp.prob[a,b,s,g,st]) <- assagst.effect[a,b,s,g,st] +
              gpcw.effect
            logit(assag.v.prob[a,b,s,g,st]) <- assagst.effect[a,b,s,g,st] +
              vcw.effect
            logit(assag.o.prob[a,b,s,g,st]) <- assagst.effect[a,b,s,g,st] +
              ocw.effect
          }
        }
      }
  ...
  #monitor# full.pd, dic, deviance, a.prob, prob.of.bad.hosp, prob.of.bad.gp, prob.of.bad.vol, prob.of.bad.out, bad.p, bad.gp, bad.v, bad.o, intercept, sd.class, sd.sampleseason, sd.agegroup, sd.ward, sd.st
}
